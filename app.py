...
if opcion == "Ventas":
    ...
    with tabs[0]: # REGISTRAR
        ...
        # 2. CAMPOS CON FORMATO AUTOMÁTICO (REGISTRO)
        v_tot = c7.currency_input("Total ($ COP)", symbol="$", key="tr"+vs, default=0.0)
        v_abo = c8.currency_input("Abono Inicial ($ COP)", symbol="$", key="ar"+vs, default=0.0)
        ...
        if not df_v.empty:
            ...
            if orden_buscada != "Seleccionar...":
                ...
                with st.form("form_edit"):
                    ...
                    # 3. CAMPOS CON FORMATO AUTOMÁTICO (EDICIÓN)
                    e_tot = col6.currency_input("Total ($ COP)", symbol="$", value=float(val['total_n']))
                    e_nuevo_abo = col7.currency_input("Añadir nuevo abono ($ COP)", symbol="$", default=0.0)
                    ...
                    if st.form_submit_button("💾 ACTUALIZAR TODA LA ORDEN"):
                        ...
                        payload = {
                            ...
                            "descripcion": e_desc, "total": e_tot, "abono": abono_total_acumulado,
                            "saldo": nuevo_saldo, "estado": e_est, "historial_pagos": hist_act
                        }
