...
if opcion == "Ventas":
    ...
    with tabs[0]: # REGISTRAR (TODAS LAS CASILLAS)
        ...
        c7, c8 = st.columns(2)
        v_tot = c7.number_input("Total ($)", min_value=0.0, step=100.0, key="t"+vs)
        v_abo = c8.number_input("Abono Inicial ($)", min_value=0.0, step=100.0, key="a"+vs)
    ...
    with tabs[1]: # EDITAR
        if not df_v.empty:
            ...
            if ord_s != "Seleccionar...":
                ...
                n_abo = st.number_input(f"Abonar a saldo (Actual: ${d['saldo_n']:,.0f})", min_value=0.0)
                e_est = st.selectbox("Cambiar Estado", ["EN PROCESO", "TERMINADO", "PAGADO"], index=0)
    ...
    if st.session_state['rol'] == 'admin':
        with tabs[2]: # REPORTES AVANZADOS (FILTRO DE FECHA RECUPERADO)
            st.subheader("📊 Filtros de Auditoría")
            f1, f2 = st.columns(2)
            sel_emp = f1.selectbox("👤 Empleado:", ["Todos"] + df_v['empleado'].unique().tolist())
            tipo_pago = f2.radio("💰 Cobro:", ["Todas", "Solo Pendientes", "Solo Pagadas"], horizontal=True)
            
            # FILTRO POR TIEMPO
            st.markdown("---")
            modo_t = st.radio("📅 Filtrar por Fecha:", ["Todo", "Día / Semana", "Mes / Año"], horizontal=True)
            df_r = df_v.copy()
            ...
            m1.metric("Total Ventas", f"$ {df_r['total_n'].sum():,.0f}")
    ...
    busq = st.text_input("🔍 Buscar por Orden o Cliente:")
    df_h = df_v.copy()
    if busq: df_h = df_h[df_h.apply(lambda r: busq.lower() in r.astype(str).str.lower().values, axis=1)]
...
# ... (Mantenemos tu lógica de gestión de empleados igual)
